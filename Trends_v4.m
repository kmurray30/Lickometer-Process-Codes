...
days = textread('C:/LickoMeterTemp/days2_bottom.txt','%f','delimiter','\n','whitespace',''); %#ok<*DTXTRD>
out_files = textread('C:/LickoMeterTemp/files2_bottom.txt','%s','delimiter','\n','whitespace','');
...
%Get rid of extra spaces at end
while strcmp (cfnNoTxt(end), ' ')
    cfnNoTxt(end) = '';
end
...
Trend(f,1,sw)=d;
Optimal_file_name=sprintf('%s/Switch%.f/Optimal%.f.xlsx',cfnNoTxt,sw,sw);
%[~,~,BoutInfoComp]=xlsread(sprintf('%s/Switch%.f/Bout_info%.f.xlsx',cfnNoTxt,sw,sw),'w');
%BoutInfoComp(1,2)={sprintf('Day %.f',d)};
%xlswrite(sprintf('%s/Switch%.f/Cage%.fBout_info.xlsx',trend_folder,sw,sw),BoutInfoComp,f);
...
OptimalTmpHeaders=[{'Cage'},{'Day'},{'File name'},{'Bout'},{'Number of Microstructures in bout'},{'Lick in bout'},{'On-time'},{'Off-time'},{'Timestamp'}];
xlsSizeLimit=59999;
if size(OptimalTmp,1)>=xlsSizeLimit
    for v=1:floor(size(OptimalTmp,1)./xlsSizeLimit)
    xlswrite(sprintf('%s/Switch%.f/Cage%.fOptimal%.f.xlsx',trend_folder,sw,sw,v),[OptimalTmpHeaders;OptimalTmp(((v-1)*xlsSizeLimit+1):(v*xlsSizeLimit),:)],1,sprintf('A1:G%.f',(xlsSizeLimit+1)));
    end
    xlswrite(sprintf('%s/Switch%.f/Cage%.fOptimal%.f.xlsx',trend_folder,sw,sw,v+1),[OptimalTmpHeaders;OptimalTmp((v*xlsSizeLimit+1):size(OptimalTmp,1),:)],1,sprintf('A1:G%.f',size(OptimalTmp,1)-v*(xlsSizeLimit-1)-1));
else
    xlswrite(sprintf('%s/Switch%.f/Cage%.fOptimal.xlsx',trend_folder,sw,sw),[OptimalTmpHeaders;OptimalTmp]);
end
end
...
TrendData_file=sprintf('%s/Trend%.f Data.xlsx',switch_folder,id);
