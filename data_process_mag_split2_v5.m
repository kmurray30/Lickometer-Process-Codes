...
%BufMat excel files must be in appropriate out_date folders in destination
%(LickoMeterTemp). The names of their parent text files must also be in the
%location indicated by second code line below. Need to make it so the first
%Matlab code creates folders and BufMat, and this just adds to it.
...
%Read BufMat from xls file
BufMat = xlsread(BufMat_file, 'w');
BufMat = BufMat(:,1:2);
