function magnetic_process4(out_files, destination, mincontact, maxcontact, minnocontact, maxnocontact, boutdeterminant, ms, noise, plotMS)
messagebox = msgbox('Processing...');
...
elseif exist(current_file_name,'file')==0
    msgbox('All imported files must exist. Check name of file.');
...
%Start reading data
mask = data(:,1) == id;
data1 = data(mask, 2:3);    %Separate data so that you look at one cage at a time
BufMat = [0,0];
if size(data1,1) > 1
for sw = 1:(size(data1,1) - 1)
        if sw+1 >= size(data1,1)    %Makes sure indices are never out of bounds
            break                   %and is repeated later
        end
    if data1(sw+1,2)-data1(sw,2) < noise %This block deletes rows with too little time between flickers
        %error('There are two 0s or two 1s in a row');
        data1(sw,1) = 0; %Makes it so all noise is recorded as a zero 
        delTime = data1(sw,2); %Records deleted time to make sure times are deleted until flickering stops, not just until time differences add up to be greater than noise
        while data1(sw+1,2) - delTime < noise
        delTime = data1(sw+1,2);
        data1(sw+1,:) = [];
        if sw+1 >= size(data1,1)
            break
        end
        end
    end
...
BufMat_file=sprintf('%s\\BufMat%.f.xlsx',switch_folder,id);
...
if BufMat(j,2)>=minnocontact && BufMat(j,1)<=maxcontact && BufMat(j,1)>=mincontact
...
%Chart of Microstructure Interpretations
    ...
    if strcmp(plotMS,'on');
    %Recreate 00001110010 type data for plotting
    BufDat = zeros(v,1);
    msBM = BufMat(MSPos(m,1):MSPos(m,2),:); %"Microstructure BufMat" cuts chunk of BufMat that is microstructure
    numpos=1;
    for f=1:size(msBM,1)
    for g=numpos:numpos+msBM(f,1)-1
    BufDat(g,1)=1;
    end
    numpos=numpos+msBM(f,1)+msBM(f,2);
    end
...
%    fprintf(sprintf('Microstructure exists %.f time(s) in bout %.f of switch %.f of file %s\n',data3(i,8),i,id,out_files{q}))
    step=1;
    x_axis = step:step:v*step;
    y_axis = BufDat';
    bar(x_axis,y_axis);
    xlabel(sprintf('time (ms) starting at %d ms', z));
    ylabel('licks');
...
if size(Optimal,1)>0
Optimal_file=sprintf('%s\\Optimal%.f.xlsx',switch_folder,id);
warning('off','MATLAB:xlswrite:AddSheet');
...
TrendData_file=sprintf('%s\\Trend%.f_Data.xlsx',switch_folder,id);
