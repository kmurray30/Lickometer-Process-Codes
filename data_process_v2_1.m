...
for opt=1:size(MSPos,1)
    MSLicksInBout=[MSLicksInBout;(BufMat(MSPos(opt,1):MSPos(opt,2),1:2))];
    Optimal=[Optimal;num2cell(zeros((MSPos(opt,2)-MSPos(opt,1)+1),5)),num2cell((BufMat(MSPos(opt,1):MSPos(opt,2),1:2)))];              %creates new rows for the comprehensive chart
