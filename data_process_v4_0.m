...
out_files = textread('C:\LickoMeterTemp\files5.txt','%s','delimiter','\n','whitespace','');
...
%Chart of Microstructure Interpretations
    ...
    w=sum(BufMat(MSPos(m,1):(MSPos(m,2)-1),2))/(y-1);
    v=sum(sum(BufMat(MSPos(m,1):MSPos(m,2),:)))-BufMat(MSPos(m,2),2);
%     if BufMat(MSPos(m,2),2)>maxnocontact
